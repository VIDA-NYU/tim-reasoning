...
class RuleBasedClassifier:
    ...
    def is_mistake(self, current_step, detected_action, threshold=0.8):
        ...
        logger.info(f'Pair of (action, object) for the recipe step: {str(step_action_pairs)}')
        logger.info(f'Pair of (action, object) for the detected actions: {str(detected_action_pairs)}')
        ...
        for step_action, step_object in step_action_pairs:
            ...
            for detected_action, detected_object in detected_action_pairs:
                ...
                logger.info(f'Similarity between "{step_text}" and "{detected_text}": {similarity:.2f}')
